javascript
// Add teacherWallet to props
export default function TeacherDashboard({ activeSession, setActiveSession, teacherWallet }) {

  // In endSession, replace the wallet connection block with:
  const { wallet, walletAddress, connectWallet } = teacherWallet;

  if (!wallet) {
    alert("Please connect your teacher wallet using the Connect Lace Wallet button at the top of the page.");
    setSubmittingTx(false);
    return;
  }

  // Then use wallet directly — no need to call BrowserWallet.enable() again
